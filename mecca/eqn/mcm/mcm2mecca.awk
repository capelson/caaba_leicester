...

{
    ...
    if (DEFVAR) {
        #  mz_rs_20200906+
        # keep case of MCM variables unchanged
    ...
    if (EQUATION) {
        if (match($1,"[0-9]+",Rnumber)) {
            ...
            gnum = sprintf("<G_MCM_%05d>", Rnumber[0])
            # for species that also exist in MECCA, use upper/lower case as in MECCA:
            gsub(" CL ",      " Cl ")
            gsub(" CH3CL ",   " CH3Cl ")
            gsub(" CH2CL2 ",  " CH2Cl2 ")
            gsub(" CHCL3 ",   " CHCl3 ")
            gsub(" CH3CCL3 ", " CH3CCl3 ")
            gsub(" CH3BR ",   " CH3Br ")
            gsub(" HCL ",     " HCl ")
            gsub(" BR ",      " Br ")
            # mz_rs_20200906-
            ...
            if (match($0,/\yCClCONO3\y\s*\=\s*\yCHOCl\y\s*\+\s*\yCO\y\s*\+\s*\yHO2\y\s*\+\s*\yNO2\y/)) {
                print "add DUMMY to equation to avoid multiple equation"
                print $0
                sub("=","= Dummy +")
            }
            ...
            for (i=1; i<NF;i++) {
                ...
                if (match($i,/J\([0-9]*\)/)) {
                    ...
                    gnum = sprintf("<J_MCM_%05d>", Rnumber[0])
                }
            }
