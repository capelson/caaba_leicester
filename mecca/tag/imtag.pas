...
// = imtag.aps =======================================================
// (isotopic) mechanism tagging tool, version 2
// refurbushed 150802 from im-double
//
...
writeln(f,'! ---------------- [',tagname,'] - inline code ----------------');
writeln(f,'!');
writeln(f,'! reaction rates variables (to be used in multiplied tagged reactions)');
a:='  REAL(dp) :: ';
for i:=1 to _eqs do
    if (eqs[i].itag) and (eqs[i].etag<2) then
...
writeln(f,'! configuration: ',tagname);
  write(f,'! element: ',isoelem,', ',_isos,' isotopologues of masses ');
for i:=1 to _isos do
    write(f,clsname[i],' ');
writeln(f);
                          ...
                          if (trans[e].ib=0) then           // skipping IB transfers here
                             ...
                             if not(in_utsl(trans[e].src)) then
                                ...
                                writeln(' <!> warning: source specification record (',tagname,': #',e,
                                        ') for <',abbr,'> refers to species not in used-TSL (',trans[e].src,')');
                                continue;
                                end;
                             ...
                             for k:=1 to _isos do
                                 ...
                                 // educts
                                 a:=' <'+substr(trsyntax,'@',abbr)+clsname[k]+'s'+trans[e].src+'> ';
                                     ...
                                     if (in_busl(prod[j])) then
                                        if not(in_tsl(prod[j])) then
                                           a+=format_prod(substr(ptpsyntax,'@',clsname[k]+prod[j]),stoi[j])
                                            ...
                                            if (d>0) then
                                               a+=format_prod(substr(ptpsyntax,'@',giveiso(prod[j],k,abbr)),stoi[j]*pf)
                                                 +format_prod(substr(ptpsyntax,'@',giveiso(prod[j],1,abbr)),stoi[j]*(1.0-pf)); // mz_sg_160209 fix: PTs when shifts are enabled
...
{$IFDEF USE_PT}
 ...
 {$IFDEF USE_PT_KIE}
                                 ...
                                 for j:=1 to _kie do
                                     if ((kie[j].imec) and (abbr=kie[j].abbr)) then
                                        begin
                                        a+=format_prod(substr(ptsyntax,'@',abbr+clsname[k],1);
                                        break;
                                        end;
 {$ENDIF}
{$ENDIF}
...
{$ENDIF}
    ...
    if (pos('#EQUATIONS',eqnfile[l].line)>0) then
       ...
       if (kieproc<>_nonestr) then         // if there is KIE processing file specified
          begin
          writeln(f,'{------ [imtag] - KIE process section for ',isoelem,' -------------------------------}');
...
for nconf:=1 to _conf do
    ...
    // tracer def. file
{$IFDEF TRACDEF_CHEMPROP}
    imcom_make_tracprop(extractword(1,paramstr(2),[':']),cmodel+'_'+tagname+'_chemprop.tbl');
    imcom_make_tracprop(extractword(2,paramstr(2),[':']),cmodel+'_'+tagname+'_process.tbl');
{$ELSE}
    imcom_make_tracprop(paramstr(2),tracdef);
{$ENDIF}
