...
'''
xpdflatex: eXecute pdflatex etc., automating the process of
generating a LaTeX document. Mostly based on latexmk.py from Jan Kanis
and latexmake.py from Marc Schlaich.
'''

# https://github.com/JanKanis/latexmk.py
# https://github.com/schlamar/latexmk.py (obsolete)

# a few modifications by Rolf Sander (2020):
# - works for BibTeX if *.bib is in another directory
...
def main(cmd=None):
    ...
    if (cmd):
        args, rest = parser.parse_known_args(cmd.split())
    else:
