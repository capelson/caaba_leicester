#!/usr/bin/env python3
# -*- coding: utf-8 -*- Time-stamp: <2020-11-03 15:05:39 sander>
...
#       benchmark.ini              -> only when integrator changes
...
def run_xmecca(meccainifile, integrator_name):
    # prepare MECCA config (*.ini) file:
    print('\nmodifying MECCA config (*.ini) file %s.ini' % (meccainifile))
    meccainidir = CAABADIR + '/mecca/ini/'
    with open(meccainidir + meccainifile + '.ini', 'r') as inputfile, \
         open(meccainidir + 'benchmark.ini',   'w') as outputfile:
        outputfile.write('# '+DONTEDIT+'\n')
        for line in inputfile:
            line0 = line # backup of original line
            line = re.sub('(^\s*integr\s+=\s+).*', r'\1'+integrator_name, line)
            line = re.sub('(^\s*latex\s+=\s+).*', r'\1n', line)    # latex = n
            line = re.sub('(^\s*graphviz\s+=\s+).*', r'\1n', line) # graphviz = n
            outputfile.write(line)
            if line!=line0: 
                print('<'+line0, '>'+line) # print what was changed
    print('benchmark.ini has been created\n')
    # compile_integrator:
    olddir = os.getcwd()
    print('\nrunning xmecca...', end=' ')
    with open(f'{CAABADIR}/mecca/xmecca.log', 'w') as f:
        with redirect_stdout(f):
            xmecca.exe(f'{CAABADIR}/mecca/ini/benchmark.ini')
    print('DONE')
    os.chdir(CAABADIR)
...
def qexec_multirun(full_nc_file, multirun_outputdir, qexec=False):
    ...
    shutil.copy(CAABADIR + '/mecca/ini/benchmark.ini', multirun_outputdir)
...
def do_runs(icntrl_dict, rcntrl_dict, meccainifile, integrators = list(),
        ...
        if integrators is not None:
            if len(integrators) - 1 >= index:
                if integrators[index] != '_':
                    run_xmecca(meccainifile, integrators[index])
...
def parse_command_line_arguments():
    ...
    parser.add_argument('-p', '--plot', action='store', type=str, nargs='?',
                        ...
                        'to load from.')
    parser.add_argument('-b', '--meccainifile', action='store', type=str, default='mbl',
                        help='meccainifile for xmecca')
...
def main():
    ...
    print(integrators)
    print('meccainifile:')
    if (os.path.isfile(CAABADIR + '/mecca/ini/' + args.meccainifile + '.ini')):
        print(args.meccainifile + '.ini')
    else:
        sys.exit('ERROR: Cannot find ' + args.meccainifile + '.ini')
    print('netCDF input file:')
    print(args.input)

    outputdir = do_runs(icntrl_dict, rcntrl_dict, args.meccainifile,
